...
rb_iseq_compile_node(rb_iseq_t *iseq, NODE *node)
{
	...
	ADD_INSN2(ret, 0, getlocal, INT2FIX(LVAR_ERRINFO), INT2FIX(0));
	  ...
	  case ISEQ_ELEMENT_INSN:
		...
		for (j = 0; types[j]; j++) {
		      ...
		      case TS_CDHASH:
			{
		      ...
		      case TS_LINDEX:
			generated_iseq[code_index + 1 + j] = FIX2INT(operands[j]) + (VM_ENV_MANAGE_DATA_SIZE - 1 /* specval */);
			break;
		      case TS_NUM:	/* ulong */
...
iseq_compile_each(rb_iseq_t *iseq, LINK_ANCHOR *ret, NODE * node, int poped)
{
	...
	while (resq) {
		...
		switch (nd_type(narg)) {
		  case NODE_ARRAY:
		    while (narg) {
			ADD_INSN2(ret, line, getlocal, INT2FIX(LVAR_ERRINFO), INT2FIX(0));
	...
	*/
	...
	if (nd_type(node) == NODE_SUPER) {
		...
		if (liseq->body->param.flags.has_kwrest) {
		    ADD_INSN2(args, line, getlocal, INT2FIX(liseq->body->local_table_size - liseq->body->param.keyword->rest_start), INT2FIX(lvar_level));
	...
	if (!poped) {
		...
		if (ip) {
		    ADD_INSN2(ret, line, getlocal, INT2FIX(LVAR_ERRINFO), INT2FIX(level));
		}
		else {
		    ADD_INSN(ret, line, putnil);
		}
...
for_self_aset(rb_iseq_t *iseq, LINK_ANCHOR *ret, VALUE a)
{
    ...
    ADD_INSN2(ret, line, getlocal, INT2FIX(numberof(vars)-1), INT2FIX(0));
    ADD_INSN1(ret, line, putobject, args->arg);
    ADD_INSN1(ret, line, opt_call_c_function, (VALUE)args->func);
    ADD_INSN(ret, line, pop);
    return Qnil;
