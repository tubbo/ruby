...
static VALUE vm_call0_body(rb_thread_t* th, struct rb_calling_info *calling, const VALUE *argv);
...
vm_call0(rb_thread_t* th, VALUE recv, ID id, int argc, const VALUE *argv, const rb_callable_method_entry_t *me)
{
    ...
    return vm_call0_body(th, calling, argv);
}

#if OPT_CALL_CFUNC_WITHOUT_FRAME
static VALUE
vm_call0_cfunc(rb_thread_t* th, struct rb_calling_info *calling, const VALUE *argv)
{
    VALUE val;

    RUBY_DTRACE_CMETHOD_ENTRY_HOOK(th, calling->cc->me->owner, calling->ci->mid);
    EXEC_EVENT_HOOK(th, RUBY_EVENT_C_CALL, calling->recv, calling->ci->mid, calling->cc->me->owner, Qnil);
    {
    ...
    EXEC_EVENT_HOOK(th, RUBY_EVENT_C_RETURN, calling->recv, calling->ci->mid, callnig->cc->me->owner, val);
    RUBY_DTRACE_CMETHOD_RETURN_HOOK(th, calling->cc->me->owner, calling->ci->mid);

    return val;
}
#else
static VALUE
vm_call0_cfunc_with_frame(rb_thread_t* th, struct rb_calling_info *calling, const VALUE *argv)
{
    VALUE val;
    const rb_callable_method_entry_t *me = calling->cc->me;
    const rb_method_cfunc_t *cfunc = &me->def->body.cfunc;
...
vm_call0_cfunc(rb_thread_t* th, struct rb_calling_info *calling, const VALUE *argv)
{
    return vm_call0_cfunc_with_frame(th, calling, argv);
}
#endif

/* `ci' should point temporal value (on stack value) */
static VALUE
vm_call0_body(rb_thread_t* th, struct rb_calling_info *calling, const VALUE *argv)
{
    VALUE ret;
  ...
  again:
      ...
      case VM_METHOD_TYPE_ISEQ:
	{
		...
		*reg_cfp->sp++ = argv[i];
	    ...
	    vm_call_iseq_setup(th, reg_cfp, calling);
	    th->cfp->flag |= VM_FRAME_FLAG_FINISH;
	    return vm_exec(th); /* CHECK_INTS in this function */
	}
      case VM_METHOD_TYPE_NOTIMPLEMENTED:
      case VM_METHOD_TYPE_CFUNC:
	ret = vm_call0_cfunc(th, calling, argv);
	goto success;
      case VM_METHOD_TYPE_ATTRSET:
	rb_check_arity(calling->argc, 1, 1);
	ret = rb_ivar_set(calling->recv, calling->cc->me->def->body.attr.id, argv[0]);
	goto success;
      case VM_METHOD_TYPE_IVAR:
	rb_check_arity(calling->argc, 0, 0);
	ret = rb_attr_get(calling->recv, calling->cc->me->def->body.attr.id);
	goto success;
      case VM_METHOD_TYPE_BMETHOD:
	ret = vm_call_bmethod_body(th, calling, argv);
	goto success;
      case VM_METHOD_TYPE_ZSUPER:
      case VM_METHOD_TYPE_REFINED:
	{
	    const rb_method_type_t type = calling->cc->me->def->type;
	    VALUE super_class;

	    if (type == VM_METHOD_TYPE_REFINED && calling->cc->me->def->body.refined.orig_me) {
		calling->cc->me = refined_method_callable_without_refinement(calling->cc->me);
		goto again;
	    }

	    super_class = RCLASS_SUPER(calling->cc->me->defined_class);

	    if (!super_class || !(calling->cc->me = rb_callable_method_entry(super_class, calling->ci->mid))) {
		enum method_missing_reason ex = (type == VM_METHOD_TYPE_ZSUPER) ? MISSING_SUPER : 0;
		ret = method_missing(calling->recv, calling->ci->mid, calling->argc, argv, ex);
		goto success;
	    }
	...
	calling->cc->me = aliased_callable_method_entry(calling->cc->me);
	goto again;
      case VM_METHOD_TYPE_MISSING:
	{
	    VALUE new_args = rb_ary_new4(calling->argc, argv);

	    rb_ary_unshift(new_args, ID2SYM(calling->ci->mid));
	...
	switch (calling->cc->me->def->body.optimize_type) {
	  ...
	  default:
	    rb_bug("vm_call0: unsupported optimized method type (%d)", calling->cc->me->def->body.optimize_type);
	}
	break;
      case VM_METHOD_TYPE_UNDEF:
	break;
    }
    rb_bug("vm_call0: unsupported method type (%d)", calling->cc->me->def->type);
    return Qundef;
