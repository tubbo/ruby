...
static inline VALUE
vm_getivar(VALUE obj, ID id, IC ic, struct rb_calling_info *calling, int is_attr)
{
#if USE_IC_FOR_IVAR
	...
	if (LIKELY(is_attr ? calling->cc->aux.index > 0 : ic->ic_serial == RCLASS_SERIAL(klass))) {
	    long index = !is_attr ? (long)ic->ic_value.index : (long)(calling->cc->aux.index - 1);
		...
		if (st_lookup(iv_index_tbl, id, &index)) {
			...
			calling->cc->aux.index = (int)index + 1;
		    }
		}
	    }
	}
...
static inline VALUE
vm_setivar(VALUE obj, ID id, VALUE val, IC ic, struct rb_calling_info *calling, int is_attr)
{
#if USE_IC_FOR_IVAR
    rb_check_frozen(obj);
	...
	if (LIKELY(
	    ...
	    (is_attr && calling->cc->aux.index > 0))) {
	    long index = !is_attr ? (long)ic->ic_value.index : (long)calling->cc->aux.index-1;
...
static VALUE vm_call_general(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling);
...
static inline VALUE vm_call_iseq_setup_2(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling, int opt_pc);
static inline VALUE vm_call_iseq_setup_normal(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling, int opt_pc);
static inline VALUE vm_call_iseq_setup_tailcall(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling, int opt_pc);
static VALUE vm_call_iseq_setup_normal_0start(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling);
static VALUE vm_call_iseq_setup_tailcall_0start(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling);
...
static inline int
vm_callee_setup_block_arg(rb_thread_t *th, struct rb_calling_info *calling, const rb_iseq_t *iseq, VALUE *argv, const enum arg_setup_type arg_setup_type)
{
    if (LIKELY(simple_iseq_p(iseq))) {
	rb_control_frame_t *cfp = th->cfp;
	VALUE arg0;

	CALLER_SETUP_ARG(cfp, calling); /* splat arg */
	...
	return setup_parameters_complex(th, iseq, calling, argv, arg_setup_type);
    }
}

static inline int
vm_callee_setup_arg(rb_thread_t *th, struct rb_calling_info *calling, const rb_iseq_t *iseq, VALUE *argv)
{
    if (LIKELY(simple_iseq_p(iseq))) {
	rb_control_frame_t *cfp = th->cfp;
	...
	CI_SET_FASTPATH(calling,
			(UNLIKELY(calling->ci->flag & VM_CALL_TAILCALL) ? vm_call_iseq_setup_tailcall_0start :
			                                                  vm_call_iseq_setup_normal_0start),
			(!IS_ARGS_SPLAT(calling) && !IS_ARGS_KEYWORD(calling) &&
			 !(METHOD_ENTRY_VISI(calling->cc->me) == METHOD_VISI_PROTECTED)));
	return 0;
    }
    else {
	return setup_parameters_complex(th, iseq, calling, argv, arg_setup_method);
    }
}
...
static VALUE
vm_call_iseq_setup(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling)
{
    int opt_pc = vm_callee_setup_arg(th, calling, def_iseq_ptr(calling->cc->me->def), cfp->sp - calling->argc);
    return vm_call_iseq_setup_2(th, cfp, calling, opt_pc);
}

static inline VALUE
vm_call_iseq_setup_2(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling, int opt_pc)
{
    if (LIKELY(!(calling->ci->flag & VM_CALL_TAILCALL))) {
	return vm_call_iseq_setup_normal(th, cfp, calling, opt_pc);
    }
    else {
	return vm_call_iseq_setup_tailcall(th, cfp, calling, opt_pc);
    }
}

static inline VALUE
vm_call_iseq_setup_normal(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling, int opt_pc)
{
    VALUE *argv = cfp->sp - calling->argc;
    const rb_callable_method_entry_t *me = calling->cc->me;
...
static inline VALUE
vm_call_iseq_setup_tailcall(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling, int opt_pc)
{
    unsigned int i;
    VALUE *argv = cfp->sp - calling->argc;
    const rb_callable_method_entry_t *me = calling->cc->me;
...
static VALUE
vm_call_iseq_setup_normal_0start(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling)
{
    return vm_call_iseq_setup_normal(th, cfp, calling, 0);
}

static VALUE
vm_call_iseq_setup_tailcall_0start(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling)
{
    return vm_call_iseq_setup_tailcall(th, cfp, calling, 0);
}
...
static VALUE
vm_call_cfunc_with_frame(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling)
{
    VALUE val;
    const rb_callable_method_entry_t *me = calling->cc->me;
...
vm_call_cfunc_latter(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling)
{
    ...
    VALUE recv = calling->recv;
    const rb_method_cfunc_t *cfunc = vm_method_cfunc_entry(calling->cc->me);
...
vm_call_cfunc(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling)
{
    ...
    CALLER_SETUP_ARG(reg_cfp, calling);
    if (len >= 0) rb_check_arity(calling->argc, len, len);

    RUBY_DTRACE_CMETHOD_ENTRY_HOOK(th, me->owner, me->called_id);
    EXEC_EVENT_HOOK(th, RUBY_EVENT_C_CALL, recv, me->called_id, me->owner, Qnil);

    if (!(calling->cc->me->def->flag & METHOD_VISI_PROTECTED) &&
	!(calling->ci->flag & VM_CALL_ARGS_SPLAT) &&
	!(calling->ci->kw_arg != NULL)) {
	CI_SET_FASTPATH(calling, vm_call_cfunc_latter, 1);
    }
...
rb_vm_call_cfunc_push_frame(rb_thread_t *th)
{
    ...
    vm_push_frame(th, 0, VM_FRAME_MAGIC_CFUNC,
		  ...
		  0, th->cfp->sp + calling->cc->aux.inc_sp, 1, 0);
...
{
    CALLER_SETUP_ARG(reg_cfp, calling);
    return vm_call_cfunc_with_frame(th, reg_cfp, calling);
}
#endif

static VALUE
vm_call_ivar(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling)
{
    VALUE val = vm_getivar(calling->recv, calling->cc->me->def->body.attr.id, 0, calling, 1);
    cfp->sp -= 1;
    return val;
}

static VALUE
vm_call_attrset(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling)
{
    VALUE val = vm_setivar(calling->recv, calling->cc->me->def->body.attr.id, *(cfp->sp - 1), 0, calling, 1);
    cfp->sp -= 2;
    return val;
}

static inline VALUE
vm_call_bmethod_body(rb_thread_t *th, struct rb_calling_info *calling, const VALUE *argv)
{
    rb_proc_t *proc;
    VALUE val;

    /* control block frame */
    th->passed_bmethod_me = calling->cc->me;
    GetProcPtr(calling->cc->me->def->body.proc, proc);
...
static VALUE
vm_call_bmethod(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling)
{
    VALUE *argv;
    int argc;

    CALLER_SETUP_ARG(cfp, calling);
    ...
    return vm_call_bmethod_body(th, calling, argv);
}
...
inline
#endif
VALUE vm_call_method(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling);

static VALUE
vm_call_opt_send(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling)
{
    int i;
    VALUE sym;
	...
	ci_entry = *(struct rb_call_info_with_kwarg *)calling->ci;
    }
    else {
	ci = &ci_entry.ci;
	ci_entry.ci = *calling->ci;
    }
	...
	th->method_missing_reason = calling->cc->aux.method_missing_reason = ci_missing_reason(ci);
    }
    else {
	/* shift arguments */
	if (i > 0) {
    ...
    calling->cc->me = rb_callable_method_entry_without_refinements(CLASS_OF(calling->recv), ci->mid);
    ci->flag = VM_CALL_FCALL | VM_CALL_OPT_SEND;
    return vm_call_method(th, reg_cfp, calling);
}

static VALUE
vm_call_opt_call(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling)
{
    rb_proc_t *proc;
    int argc;
    VALUE *argv;
...
static VALUE
vm_call_method_missing(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling)
{
    VALUE *argv = STACK_ADDR_FROM_TOP(calling->argc);
    struct rb_call_info ci_entry;
    const struct rb_call_info *orig_ci = calling->ci;
    struct rb_call_cache cc_entry, *orig_cc = calling->cc;
    unsigned int argc;
...
static VALUE vm_call_super_method(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling);
...
#endif
VALUE
vm_call_method(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling)
{
    ...
    VM_ASSERT(callable_method_entry_p(calling->cc->me));
  ...
  start_method_dispatch:
    ...
    if (me != NULL) {
	if (LIKELY(METHOD_ENTRY_VISI(me) == METHOD_VISI_PUBLIC && METHOD_ENTRY_SAFE(me) == 0)) {
	    VALUE klass;
	  ...
	  normal_method_dispatch:
	    ...
	    switch (me->def->type) {
	      case VM_METHOD_TYPE_ISEQ:{
		CI_SET_FASTPATH(calling, vm_call_iseq_setup, enable_fastpath);
		return vm_call_iseq_setup(th, cfp, calling);
	      }
	      case VM_METHOD_TYPE_NOTIMPLEMENTED:
	      case VM_METHOD_TYPE_CFUNC:
		CI_SET_FASTPATH(calling, vm_call_cfunc, enable_fastpath);
		return vm_call_cfunc(th, cfp, calling);
	      case VM_METHOD_TYPE_ATTRSET:{
		CALLER_SETUP_ARG(cfp, calling);
		rb_check_arity(calling->argc, 1, 1);
		calling->cc->aux.index = 0;
		CI_SET_FASTPATH(calling, vm_call_attrset, enable_fastpath && !(calling->ci->flag & VM_CALL_ARGS_SPLAT));
		return vm_call_attrset(th, cfp, calling);
	      }
	      case VM_METHOD_TYPE_IVAR:{
		CALLER_SETUP_ARG(cfp, calling);
		rb_check_arity(calling->argc, 0, 0);
		calling->cc->aux.index = 0;
		CI_SET_FASTPATH(calling, vm_call_ivar, enable_fastpath && !(calling->ci->flag & VM_CALL_ARGS_SPLAT));
		return vm_call_ivar(th, cfp, calling);
	      }
	      case VM_METHOD_TYPE_MISSING:{
		calling->cc->aux.method_missing_reason = 0;
		CI_SET_FASTPATH(calling, vm_call_method_missing, enable_fastpath);
		return vm_call_method_missing(th, cfp, calling);
	      }
	      case VM_METHOD_TYPE_BMETHOD:{
		CI_SET_FASTPATH(calling, vm_call_bmethod, enable_fastpath);
		return vm_call_bmethod(th, cfp, calling);
	      }
	      case VM_METHOD_TYPE_ZSUPER:
		{
		  ...
		  zsuper_method_dispatch:
		    klass = RCLASS_SUPER(klass);
		    if (!klass) {
			me = calling->cc->me = NULL;
			goto start_method_dispatch;
		    }
		    else {
			if (calling->ci->flag & VM_CALL_KWARG) {
			    ci_temp = *(struct rb_call_info_with_kwarg *)calling->ci;
			}
			else {
			    ci_temp.ci = *calling->ci;
			}

			calling->ci = &ci_temp.ci;
			cc_temp = *calling->cc;
			calling->cc = &cc_temp;
			me = calling->cc->me = rb_callable_method_entry(klass, calling->ci->mid);
		...
		me = calling->cc->me = aliased_callable_method_entry(me);
		VM_ASSERT(me != NULL);
		goto normal_method_dispatch;
	      case VM_METHOD_TYPE_OPTIMIZED:{
		switch (me->def->body.optimize_type) {
		  case OPTIMIZED_METHOD_TYPE_SEND:
		    CI_SET_FASTPATH(calling, vm_call_opt_send, enable_fastpath);
		    return vm_call_opt_send(th, cfp, calling);
		  case OPTIMIZED_METHOD_TYPE_CALL:
		    CI_SET_FASTPATH(calling, vm_call_opt_call, enable_fastpath);
		    return vm_call_opt_call(th, cfp, calling);
		  default:
		...
		ref_me = rb_callable_method_entry(refinement, calling->ci->mid);
		if (ref_me) {
		    if (calling->cc->call == vm_call_super_method) {
		...
		if (calling->cc->me->def->body.refined.orig_me) {
		    me = calling->cc->me = refined_method_callable_without_refinement(me);
		    ...
		    klass = calling->cc->me->owner;
		    goto zsuper_method_dispatch;
		}
	      }
	    }
	    rb_bug("vm_call_method: unsupported method type (%d)", calling->cc->me->def->type);
	}
	else {
	    int safe;
	    if (!(calling->ci->flag & VM_CALL_FCALL) && (METHOD_ENTRY_VISI(calling->cc->me) == METHOD_VISI_PRIVATE)) {
		enum method_missing_reason stat = MISSING_PRIVATE;
		if (calling->ci->flag & VM_CALL_VCALL) stat |= MISSING_VCALL;

		calling->cc->aux.method_missing_reason = stat;
		CI_SET_FASTPATH(calling, vm_call_method_missing, 1);
		return vm_call_method_missing(th, cfp, calling);
	    }
	    else if (!(calling->ci->flag & VM_CALL_OPT_SEND) && (METHOD_ENTRY_VISI(calling->cc->me) == METHOD_VISI_PROTECTED)) {
		if (!rb_obj_is_kind_of(cfp->self, calling->cc->me->defined_class)) {
		    calling->cc->aux.method_missing_reason = MISSING_PROTECTED;
		    return vm_call_method_missing(th, cfp, calling);
		}
		else {
		    enable_fastpath = 0;
	    ...
	    else if ((safe = METHOD_ENTRY_SAFE(calling->cc->me)) > th->safe_level && safe > 2) {
		rb_raise(rb_eSecurityError, "calling insecure method: %"PRIsVALUE, rb_id2str(calling->ci->mid));
	    }
	    else {
		VM_ASSERT(me != NULL);
		goto normal_method_dispatch;
	...
	/* method missing */
	const int stat = ci_missing_reason(calling->ci);
	if (calling->ci->mid == idMethodMissing) {
...
static VALUE
vm_call_general(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling)
{
    return vm_call_method(th, reg_cfp, calling);
}

static VALUE
vm_call_super_method(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling)
{
    return vm_call_method(th, reg_cfp, calling);
}
...
vm_search_super_method(rb_thread_t *th, rb_control_frame_t *reg_cfp,
		       struct rb_calling_info *calling, struct rb_call_info *ci)
{
	...
	calling->cc->aux.method_missing_reason = MISSING_SUPER;
	CI_SET_FASTPATH(calling, vm_call_method_missing, 1);
    }
    else {
	/* TODO: use inline cache */
	calling->cc->me = rb_callable_method_entry(klass, ci->mid);
	CI_SET_FASTPATH(calling, vm_call_super_method, 1);
    }
}
...
static int
vm_yield_callee_setup_arg(rb_thread_t *th, struct rb_calling_info *calling, const rb_iseq_t *iseq, VALUE *argv, enum arg_setup_type arg_setup_type)
{
    return vm_callee_setup_block_arg(th, calling, iseq, argv, arg_setup_type);
}
...
vm_yield_setup_args(rb_thread_t *th, const rb_iseq_t *iseq, const int argc, VALUE *argv, const rb_block_t *blockptr, enum arg_setup_type arg_setup_type)
{
    ...
    return vm_yield_callee_setup_arg(th, calling, iseq, argv, arg_setup_type);
}

/* ruby iseq -> ruby block iseq */
static VALUE
vm_invoke_block(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling)
{
	...
	int opt_pc = vm_yield_callee_setup_arg(th, calling, iseq, rsp, is_lambda ? arg_setup_lambda : arg_setup_block);
	...
	VALUE val;
	int argc;
	CALLER_SETUP_ARG(th->cfp, calling);
	argc = calling->argc;
