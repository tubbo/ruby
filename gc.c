...
gc_marks_start(rb_objspace_t *objspace, int full_mark)
{
    ...
    if (full_mark) {
#if GC_ENABLE_INCREMENTAL_MARK
	objspace->rincgc.step_slots = (objspace->marked_slots * 2) / (objspace->rincgc.pooled_slots / HEAP_OBJ_LIMIT);

	if (1) fprintf(stderr, "objspace->marked_slots: %d, objspace->rincgc.pooled_page_num: %d, objspace->rincgc.step_slots: %d, \n",
