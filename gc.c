...
#define GC_ENABLE_INCREMENTAL_MARK 0
#endif
...
struct heap_page {
    ...
    bits_t marking_bits[HEAP_BITMAP_LIMIT]; /* for RincGC */
#endif
};
...
static int gc_mark_stacked_objects(rb_objspace_t *, int incremental);
...
#define RVALUE_REMEMBERED_BITMAP(obj)     MARKED_IN_BITMAP(GET_HEAP_REMEMBERSET_BITS(obj), (obj))
...
check_rvalue_consistency(const VALUE obj)
{
	...
	if (promoted_flag) {
		...
		rb_bug("check_rvalue_consistency: %s is not WB protected, but %s object.", obj_info(obj), type);
	    }

#if !RGENGC_AGE2_PROMOTION
	    if (!old_flag) {
		rb_bug("check_rvalue_consistency: %s is promoted, but is not old.", obj_info(obj));
	    }
#endif

	    if (old_flag && !is_full_marking(objspace) && RVALUE_MARK_BITMAP(obj) == 0) {
		rb_bug("check_rvalue_consistency: %s is old, but is not marked while minor marking.", obj_info(obj));
	    }
	}
	else {
	    if (old_flag) {
		rb_bug("check_rvalue_consistency: %s is not promoted, but is old by bitmap.", obj_info(obj));
	    }
	}
...
RVALUE_PROMOTE_INFANT(rb_objspace_t *objspace, VALUE obj, int add)
{
    ...
    if (RGENGC_CHECK_MODE && !RVALUE_INFANT_P(obj)) rb_bug("RVALUE_PROMOTE_INFANT: %s is not infant object.", obj_info(obj));
...
RVALUE_PROMOTE_YOUNG(rb_objspace_t *objspace, VALUE obj)
{
	...
	rb_bug("RVALUE_PROMOTE_YOUNG: %s is not young object.", obj_info(obj));
    }
...
{
    if (RGENGC_CHECK_MODE && !RVALUE_YOUNG_P(obj)) {
	rb_bug("RVALUE_DEMOTE_FROM_YOUNG: %s is not young object.", obj_info(obj));
    }
...
#define MEASURE_LINE(expr) do { \
    tick_t start_time = tick(); \
    expr; \
    fprintf(stderr, "0\t%lld\t%s\n", tick() - start_time, #expr); \
} while (0)
#else /* USE_TICK */
...
static inline void
gc_record(int direction, const char *event)
{
...
gc_sweep_continue(rb_objspace_t *objspace, rb_heap_t *heap)
{
    ...
    if (!GC_ENABLE_INCREMENTAL_MARK /* TODO: cheet */ && heap_increment(objspace, heap)) {
	gc_report(1, objspace, "gc_sweep_continue: success heap_increment().\n");
    }
...
gc_marks_continue(rb_objspace_t *objspace, rb_heap_t *heap)
{
	...
	gc_marks_step(objspace, slots * 4);
    }
    else {
...
heap_prepare(rb_objspace_t *objspace, rb_heap_t *heap)
{
	...
	(GC_ENABLE_INCREMENTAL_MARK /* TODO: cheet */ || heap_increment(objspace, heap) == FALSE) &&
...
newobj_of(VALUE klass, VALUE flags, VALUE v1, VALUE v2, VALUE v3)
{
	...
	assert(RVALUE_REMEMBERED(obj) == FALSE);
    ...
    if (RVALUE_PROMOTED_P(obj)) rb_bug("newobj: %s is promoted.", obj_info(obj));
...
#if USE_RGENGC
    if (RVALUE_OLD_P(obj))          CLEAR_IN_BITMAP(GET_HEAP_OLDGEN_BITS(obj), obj);
    if (RVALUE_REMEMBERED(obj))     CLEAR_IN_BITMAP(GET_HEAP_REMEMBERSET_BITS(obj), obj);
...
gc_page_sweep(rb_objspace_t *objspace, rb_heap_t *heap, struct heap_page *sweep_page)
{
    ...
    if (0) fprintf(stderr, "gc_page_sweep(%d): freed?: %d, limt: %d, freed_slots: %d, empty_slots: %d, final_slots: %d\n",
		   (int)rb_gc_count(),
		   final_slots + freed_slots + empty_slots == sweep_page->total_slots,
		   (int)sweep_page->total_slots, (int)freed_slots, (int)empty_slots, (int)final_slots);
    ...
    if (final_slots + freed_slots + empty_slots == sweep_page->total_slots) {
...
gc_sweep_start(rb_objspace_t *objspace)
{
    ...
    gc_report(1, objspace, "gc_sweep_start\n");
...
gc_sweep_step(rb_objspace_t *objspace, rb_heap_t *heap)
{
    ...
    int need_pool = GC_ENABLE_INCREMENTAL_MARK ? TRUE : FALSE;
	...
	if (gc_page_sweep(objspace, heap, sweep_page)) {
	    if (need_pool) {
		need_pool = FALSE;
...
rgengc_check_relation(rb_objspace_t *objspace, VALUE obj)
{
	...
	if (RVALUE_WB_UNPROTECTED(obj)) {
		...
		gc_report(2, objspace, "relation: (O->S) %s\n", obj_info(obj));
		...
		if (!RVALUE_OLD_P(obj)) {
		    ...
		    gc_report(2, objspace, "relation: (marked !O) %s\n", obj_info(obj));
...
{
    if (RGENGC_CHECK_MODE) assert(RVALUE_MARKED(obj));
...
gc_mark_children(rb_objspace_t *objspace, VALUE obj)
{
	...
	switch (nd_type(obj)) {
	  ...
	  case NODE_BLOCK_PASS:
	    gc_mark(objspace, (VALUE)any->as.node.u2.node);
	    /* fall through */
	  case NODE_BLOCK:	/* 1,3 */
	  case NODE_ARRAY:
	  case NODE_DSTR:
	  ...
	  case NODE_DEFS:
	  case NODE_OP_ASGN1:
	    gc_mark(objspace, (VALUE)any->as.node.u1.node);
	    /* fall through */
	  case NODE_SUPER:	/* 3 */
	  case NODE_FCALL:
	  case NODE_DEFN:
	  case NODE_ARGS_AUX:
	    gc_mark(objspace, (VALUE)any->as.node.u3.node);
	    break;
	  ...
	  case NODE_ARGS:	/* custom */
	    {
		struct rb_args_info *args = any->as.node.u3.args;
		if (args) {
	  ...
	  case NODE_ALLOCA:
	    mark_locations_array(objspace,
				 (VALUE*)any->as.node.u1.value,
				 any->as.node.u3.cnt);
	    gc_mark(objspace, (VALUE)any->as.node.u2.node);
	    break;

	  case NODE_CREF:
	    gc_mark(objspace, any->as.node.nd_refinements);
	    gc_mark(objspace, (VALUE)any->as.node.nd_clss);
	    gc_mark(objspace, (VALUE)any->as.node.nd_next);
	    break;

	  default:		/* unlisted NODE */
	    gc_mark_maybe(objspace, (VALUE)any->as.node.u1.node);
	    gc_mark_maybe(objspace, (VALUE)any->as.node.u2.node);
	    gc_mark_maybe(objspace, (VALUE)any->as.node.u3.node);
	}
 ...
 */
static inline int
gc_mark_stacked_objects(rb_objspace_t *objspace, int incremental)
{
	...
	if (incremental) {
	    ...
	    if (RGENGC_CHECK_MODE && RVALUE_MARKING(obj)) {
		rb_bug("gc_mark_stacked_objects: not incremental, but marking bit is not 0");
	    }
	}
    }
...
check_generation_i(const VALUE child, void *ptr)
{
	...
	if (!MARKED_IN_BITMAP(GET_HEAP_PAGE(parent)->rememberset_bits, parent) &&
	    ...
	    fprintf(stderr, "verify_internal_consistency_reachable_i: WB miss (O->N) %s -> %s\n",
...
verify_internal_consistency_i(void *page_start, void *page_end, size_t stride, void *ptr)
{
	...
	if (is_live_object(objspace, obj)) {
		...
		if (RVALUE_MARKING(obj)) {
		    rb_bug("verify_internal_consistency_i: %s is markng, but not in incremental marking.", obj_info(obj));
		}
	    }
#endif
	}
		...
		assert(RBASIC(obj)->flags == T_ZOMBIE);
...
gc_marks_finish(rb_objspace_t *objspace)
{
	...
	MEASURE_LINE(gc_mark_roots(objspace, 0));
	...
	MEASURE_LINE(gc_marks_wb_unprotected_objects(objspace));
    }
...
#endif
    gc_report(1, objspace, "gc_marks_finish\n");
...
gc_marks_step(rb_objspace_t *objspace, int slots)
{
    ...
    if (gc_mark_stacked_objects(objspace, slots)) {
	if (heap_eden->free_pages) {
	    /* do not finish yet */
	    gc_report(1, objspace, "gc_marks_step: the mark stack is empty, but there are more free pages.\n");
	}
	else if (gc_marks_finish(objspace)) {
	    /* finish */
	    gc_sweep(objspace, objspace->rincgc.immediate_sweep);
	}
    }
}
...
gc_marks_rest(rb_objspace_t *objspace)
{
    ...
    if (is_incremental_marking(objspace)) {
	do {
	    while (gc_mark_stacked_objects(objspace, INT_MAX) == FALSE);
...
rgengc_remembersetbits_get(rb_objspace_t *objspace, VALUE obj)
{
    ...
    return MARKED_IN_BITMAP(bits, obj) != 0;
}
...
rgengc_rememberset_mark(rb_objspace_t *objspace, rb_heap_t *heap)
{
	...
	for (j=0; j < HEAP_BITMAP_LIMIT; j++) {
		...
		do {
			...
			if (RVALUE_WB_UNPROTECTED((VALUE)p) == 0) {
			    gc_report(2, objspace, "rgengc_rememberset_mark: clear %s\n", obj_info((VALUE)p));
#if RGENGC_AGE2_PROMOTION
    ...
    gc_report(2, objspace, "rgengc_rememberset_mark: finished\n");

#if RGENGC_PROFILE > 0
    gc_report(2, objspace, "rgengc_rememberset_mark: clear_count: %"PRIdSIZE", shady_object_count: %"PRIdSIZE"\n", clear_count, shady_object_count);
    if (gc_prof_record(objspace)) {
	gc_profile_record *record = gc_prof_record(objspace);
...
rb_gc_writebarrier(VALUE a, VALUE b)
{
    ...
    gc_report(2, objspace, "rb_gc_writebarrier: [LG] %s -> %s\n", obj_info(a), obj_info(b));

    if (!is_incremental_marking(objspace)) {
      generational:
	if (!RVALUE_OLD_P(b) && RVALUE_OLD_P(a)) {
	    if (!rgengc_remembered(objspace, a)) {
		gc_report(2, objspace, "rb_gc_writebarrier: [RS] %s -> %s\n", obj_info(a), obj_info(b));
...
garbage_collect(rb_objspace_t *objspace, int full_mark, int immediate_sweep, int reason)
{
    ...
    return gc_start(objspace, full_mark, immediate_sweep, reason);
}

static int
gc_start(rb_objspace_t *objspace, int full_mark, int immediate_sweep, int reason)
{
...
#define C(c, s) ((c) != 0 ? (s) : " ")
    ...
    snprintf(buff, OBJ_INFO_BUFFERS_SIZE, "%p [%s%s%s%s%s] %s", (void *)obj,
	     RVALUE_OLD_P_RAW(obj) ? "O" : FL_TEST2(obj, FL_PROMOTED) ? "P" : " ",
	     C(RVALUE_MARK_BITMAP(obj),           "M"),
	     C(RVALUE_MARKING_BITMAP(obj),        "I"),
	     C(RVALUE_REMEMBERED_BITMAP(obj),     "R"),
