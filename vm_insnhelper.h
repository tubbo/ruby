...
#define CALL_METHOD(calling) do { \
    VALUE v = (*(calling)->cc->call)(th, GET_CFP(), (calling)); \
    if (v == Qundef) { \
	RESTORE_REGS(); \
	NEXT_INSN(); \
    } \
...
#define CI_SET_FASTPATH(calling, func, enabled) do { \
    if (LIKELY(enabled)) ((calling)->cc->call = (func)); \
} while (0)
#else
...
#define CALL_SIMPLE_METHOD(recv_) do { \
    ...
    vm_search_method(calling.ci = ci, calling.cc = cc, calling.recv = (recv_)); \
    CALL_METHOD(&calling); \
} while (0)
